...
class TelegramProvider(StorageProvider):
    ...
    async def connect(self) -> None:
        ...
        # Telethon can use this integer directly without needing to resolve the entity.
        if STORAGE_CHANNEL_ID:
            try:
                ...
                if str(STORAGE_CHANNEL_ID).lstrip('-').isdigit():
                    ...
                    log.info("SYSTEM: Storage channel ID set to %s", self._storage_channel_id)
                else:
    ...
    async def upload_file(
        ...
        return await upload_chunks(
            self.client, self._storage_channel_id, chunk_paths,
    ...
    async def download_file(
        ...
        return await download_chunks(
            self.client, self._storage_channel_id, msg_ids,
            rate_limiter=self.rate_limiter,
            dest_dir=dest_dir,
        )
    ...
    async def delete_file(
        ...
        await delete_messages(
            self.client, self._storage_channel_id, msg_ids, self.rate_limiter,
        )
    ...
    async def send_message(self, folder_name: str, text: str) -> int:
        ...
        async with self.rate_limiter:
            msg = await self.client.send_message(self._storage_channel_id, text)
        return msg.id
    ...
    async def scan_folder_messages(self, folder_name: str) -> List[dict]:
        ...
        messages = []
        async for msg in self.client.iter_messages(self._storage_channel_id):
